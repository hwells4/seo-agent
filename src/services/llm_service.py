...
import agno
from agno.models import OpenAI, Anthropic, Custom, Model
...
class LLMService:
    ...
    async def generate_text(
        ...
        try:
            response = await model.generate(prompt, params or {})
    ...
    async def generate_structured_output(
        ...
        try:
            # Use Agno's structured output capabilities
            response = await model.generate_structured(prompt, output_schema, params or {})
            
            # Validate response
            if not response or not response.data:
                raise ValueError(f"Empty structured response from model {model_name}")
